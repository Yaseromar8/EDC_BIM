            ...
            {viewingDoc ? (
                ...
                <div className="docpin-content">
                    ...
                    {attachedDocs.length === 0 ? (
                            ...
                            {attachedDocs.map(doc => {
                                ...
                                return (
                                    <div key={doc.id} className="docpin-pdf-card" onClick={() => handleOpenDoc(doc)}>
                                        ...
                                        <div className="docpin-pdf-preview">
                                            {isPdf ? (
                                                <Document
                                                    file={doc.url}
                                                    loading={<div className="docpin-preview-fallback">⏳</div>}
                                                    error={<div className="docpin-preview-fallback">📕</div>}
                                                    noData={<div className="docpin-preview-fallback">📄</div>}
                                                >
                                                    <Page
                                                        pageNumber={1}
                                                        width={150}
                                                ...
                                                <div className="docpin-preview-fallback">{getFileIcon(doc.name)}</div>
                                            )}
                                        </div>
                        ...
                        <div className="docpin-doc-list-vertical">
                            {attachedDocs.map(doc => {
                                const isPdf = doc.type === 'pdf' || (doc.name && doc.name.toLowerCase().endsWith('.pdf'));
                                return (
                                    <div key={doc.id} className="docpin-list-row" onClick={() => handleOpenDoc(doc)}>
                                        <div className="col-icon-name">
                                            <span className="doc-type-icon">{getFileIcon(doc.name)}</span>
                                            <div className="doc-id-group">
                                                <span className="doc-filename">{doc.name}</span>
                                                <span className="doc-plano-titulo">
                                                    {doc.plano_titulo || (isPdf ? 'Analizando membrete...' : 'Sin título extra')}
                                                </span>
                                            </div>
                                        </div>
                                        <div className="col-actions">
                                            ...
                                            <button
                                                className="docpin-remove-btn-list"
                                                onClick={(e) => {
                                                    e.stopPropagation();
                                                    if (onRemoveDoc) onRemoveDoc(pin.id, doc.id);
                                                }}
