...
export const downloadFolderAsZip = async (folderId, modelUrn, API_BASE_URL, folderName = "Descarga_Carpeta") => {
    ...
    try {
        ...
        // Función auxiliar para actualizar el Toast en tiempo real
        const updateProgress = () => {
            toast.loading(`Empaquetando localmente... (${downloadedCount}/${totalFiles} archivos)`, { id: toastId });
        };
        ...
        const downloadPromises = manifest.map(async (fileNode) => {
            try {
                ...
                downloadedCount++;
                updateProgress(); // Sube el contador visible de la UI
            } catch (err) {
        ...
        toast.loading("Comprimiendo final... (Generando ZIP de alta velocidad)", { id: toastId });
        
        const zipBlob = await zip.generateAsync({ type: "blob", compression: "DEFLATE" });
        saveAs(zipBlob, `${folderName}.zip`);
        toast.success(`Descarga generada con éxito.`, { id: toastId });
