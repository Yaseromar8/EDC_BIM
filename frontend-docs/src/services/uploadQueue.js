 ...
 */
export async function enqueuePhoto({ id, file, pinId, modelUrn, uploadPath, captureDate, desc, location }) {
    try {
        ...
        db.close();
        console.log(`[UploadQueue] 📥 Enqueued photo ${id} (${file.name})`);
        return true;
    } catch (err) {
...
export async function processPendingUploads(onPhotoUploaded, getBackendUrl) {
    ...
    for (const item of pending) {
        try {
            console.log(`[UploadQueue] Uploading: ${item.fileName}`);
            ...
            // 3. Confirm with Custom Attributes (e.g. location, date)
            const customAttrs = {
                capture_date: item.captureDate
            };
            if (item.location) {
                customAttrs.latitude = item.location.lat;
                customAttrs.longitude = item.location.lng;
            }
