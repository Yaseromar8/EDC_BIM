...
const RfiModule = ({ project, API, user, isAdmin }) => {
  ...
  const fetchRfis = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/rfis/${encodeURIComponent(modelUrn)}`, {
  ...
  const saveRfi = async (rfiId) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/rfis/${rfiId}`, {
        method: 'PATCH',
        headers: getAuthHeaders(),
  ...
  const getSlaInfo = (fecha, fechaRespuesta, estado) => {
    ...
    const SLA_DAYS = 7; // Plazo máximo de respuesta para RFI (días calendario)
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(8);
    doc.setTextColor(100, 100, 100);
    doc.text(`Total RFIs: ${total}`, barX + barW + 4, barY + 4);
    ...
    autoTable(doc, {
      ...
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: [10, 138, 246], textColor: 255, fontStyle: 'bold', fontSize: 9, halign: 'center' },
  ...
  const linkSelectedFile = async (fileNode) => {
    ...
    if (editingId === activeRfiForLink?.id) {
      ...
      try {
        await fetch(`${API}/api/rfis/${activeRfiForLink.id}`, {
          method: 'PATCH',
          headers: getAuthHeaders(),
    ...
    <div style={{ padding: '0px 24px 16px', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, paddingTop: 16, backgroundColor: '#fff', paddingBottom: 8, borderBottom: '1px solid #eee', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <h1 style={{ fontSize: 18, fontWeight: 400, color: '#333', margin: 0 }}>Listado de RFI</h1>
          {dashFilter && (
            <span
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
          ...
          {isAdmin && (
            <button onClick={createRfi} style={{ background: '#0a8af6', color: '#fff', border: 'none', padding: '8px 16px', borderRadius: 4, cursor: 'pointer', fontWeight: 500, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
              <span>+</span> Nuevo RFI
            </button>
          )}
      ...
      <div style={{ border: '1px solid #e0e0e0', borderRadius: 8, background: '#fff', overflowX: 'auto', overflowY: 'auto', flex: 1, minHeight: 0 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 11, tableLayout: 'fixed' }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan="8" style={{ textAlign: 'center', padding: 40, color: '#888' }}>Cargando RFIs...</td></tr>
            ) : filteredRfis.length === 0 ? (
              <tr><td colSpan="8" style={{ textAlign: 'center', padding: 40, color: '#888' }}>No se encontraron RFIs. Presiona "+ Nuevo RFI" para comenzar.</td></tr>
            ) : (
                    ...
                    <td style={{ padding: '3px 10px', overflow: 'hidden' }}>
                      {isEditing ? (
                        <input
                          ...
                          placeholder="Escribir título del RFI..."
                          style={{ width: '100%', padding: '4px 10px', border: '1.5px solid #0a8af6', borderRadius: 4, outline: 'none', fontSize: 13 }}
                          autoFocus
                        />
                      ) : (
                      ...
                      {isEditing ? (
                        <div style={{ position: 'relative' }}>
                          <div
                            ...
                            style={{
                              padding: '5px 10px', border: '1.5px solid #0a8af6', borderRadius: 4,
                            ...
                            <div style={{
                              ...
                              <div style={{ padding: '8px 10px', borderTop: '1px solid #eee', display: 'flex', gap: 6 }}>
                                ...
                                <button
                                  ...
                                  style={{ background: '#0a8af6', color: '#fff', border: 'none', borderRadius: 4, padding: '4px 10px', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}
                                >
                                  +
                                </button>
                              </div>
                      ...
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        {isEditing && (
                          <button
                            ...
                            style={{ background: 'none', border: '1px dashed #0a8af6', color: '#0a8af6', borderRadius: 4, width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: 16, fontWeight: 300 }}
      ...
      {/* ── DASHBOARD DE ESTADO RFI — Enterprise Edition ── */}
            ...
            <div>
              ...
              <div style={{ marginTop: 4, background: '#fff', borderRadius: 6, padding: '4px 10px', border: '1px solid #e0e0e0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }}>
                  <span style={{ fontSize: 9, color: '#80868b', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Total RFIs</span>
              ...
              <div style={{ flex: 1, position: 'relative', background: '#f5f5f5', overflow: 'hidden' }}>
                ...
                {!isPdf && !isImage && !isVideo && (
                  <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    ...
                    <a href={previewFile.url} target="_blank" rel="noopener noreferrer" style={{ background: '#0a8af6', color: '#fff', padding: '10px 24px', borderRadius: 8, textDecoration: 'none', fontWeight: 500, fontSize: 14 }}>Descargar Archivo</a>
                  </div>
                )}
              </div>
            </div>
                ...
                <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
                  {projectNodes.map(node => (
                    ...
                    >
                      ...
                      {!node._isFolder && (
                        <span style={{ fontSize: 11, color: '#0a8af6', fontWeight: 500 }}>Seleccionar</span>
                      )}
                    </li>
                  ))}
                </ul>
