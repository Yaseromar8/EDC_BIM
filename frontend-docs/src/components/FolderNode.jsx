/**
 * FolderNode.jsx — Árbol lateral recursivo de carpetas
 * Refactorización Fase 3: Capa de Orquestación
 * Extraído de App.jsx líneas 618-926
 */
...
export default function FolderNode({ 
  ...
  const submitRename = async () => {
    ...
    if (!renameValue.trim() || renameValue.trim() === folderNameStr.replace(/\/$/, '')) { setIsRenaming(false); return; }
  ...
  const submitCreateChild = async () => {
    ...
    const tempFolder = { id: tempId, name: newChildName.trim(), fullName: newPath, parentId: nodeId, type: 'folder', _syncing: true };
    if (cacheMethods) cacheMethods.optimisticCreate(nodeId, tempFolder);
    setIsCreatingChild(false); setNewChildName(''); setExpanded(true);
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json(); toast.error('Error al crear: ' + (err.error || 'Desconocido'));
      }
    } catch (e) {
      if (cacheMethods) cacheMethods.rollbackCreate(nodeId, tempId);
  ...
  const handleToggle = (e) => {
    e.stopPropagation();
    if (!expanded && !children && cacheMethods) { cacheMethods.expandNode(nodeId, folderFullName); }
    setExpanded(!expanded);
  };

  useEffect(() => {
    if (editingNodeId && editingNodeId.source === 'sidebar' && editingNodeId.id === nodeId) {
      setIsRenaming(true); setRenameValue((folder.name || '').replace(/\/$/, '')); setEditingNodeId(null);
    }
  ...
  useEffect(() => {
    if (creatingChildParentId === nodeId) {
      ...
      setIsCreatingChild(true); setNewChildName(''); setCreatingChildParentId(null);
    }
      ...
      >
        ...
        {isRenaming ? (
          <div className="inline-edit-box" style={{ margin: '0 8px', height: 28 }} onClick={e => e.stopPropagation()}>
            <input autoFocus value={renameValue} onFocus={(e) => e.target.select()} onChange={e => setRenameValue(e.target.value)} onBlur={(e) => { if (e.relatedTarget && e.relatedTarget.closest('.inline-edit-box')) return; /* perder el foco NO confirma: mismo criterio que en la tabla */ setIsRenaming(false); }} onKeyDown={e => { if (e.key === 'Enter') submitRename(); if (e.key === 'Escape') setIsRenaming(false); }} style={{ padding: '0 4px', fontSize: 13 }} />
            <button className="btn-cancel" style={{ width: 22, height: 22, marginLeft: 2 }} onMouseDown={(e) => e.preventDefault()} onClick={(e) => { e.stopPropagation(); setIsRenaming(false); }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
            <button className="btn-submit" style={{ width: 22, height: 22, marginLeft: 2 }} onMouseDown={(e) => e.preventDefault()} onClick={(e) => { e.stopPropagation(); submitRename(); }}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></button>
          </div>
        ) : (
          <div className="tree-text">{folder.name.replace(/\/$/, '')}</div>
        )}
      </div>
      {expanded && (
        <div className="folder-children" style={{ display: 'block' }}>
          {isCreatingChild && (
            <div className="folder-tree-item child-creating" style={{ paddingLeft: `${8 + ((level + 1) * 28)}px` }}>
              ...
              <div className="inline-edit-box" style={{ flex: 1, margin: '0 8px', height: 28 }}>
                <input autoFocus value={newChildName} onChange={e => setNewChildName(e.target.value)} onBlur={(e) => { if (e.relatedTarget && e.relatedTarget.closest('.inline-edit-box')) return; submitCreateChild(); }} onKeyDown={e => { if (e.key === 'Enter') submitCreateChild(); if (e.key === 'Escape') setIsCreatingChild(false); }} style={{ padding: '0 4px', fontSize: 13, width: '100%' }} placeholder="Nombre de carpeta..." />
                <button className="btn-cancel" style={{ width: 22, height: 22, marginLeft: 2 }} onMouseDown={(e) => e.preventDefault()} onClick={() => setIsCreatingChild(false)}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button>
                <button className="btn-submit" style={{ width: 22, height: 22, marginLeft: 2 }} onMouseDown={(e) => e.preventDefault()} onClick={submitCreateChild}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></button>
              </div>
            </div>
          )}
          {children && children.map(child => (
            <FolderNode 
              ...
              creatingChildParentId={creatingChildParentId} setCreatingChildParentId={setCreatingChildParentId}
