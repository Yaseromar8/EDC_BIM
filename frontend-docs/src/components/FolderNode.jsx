      ...
      >
        ...
        {isRenaming ? (
          <div className="inline-edit-box" style={{ margin: '0 8px', height: 28 }} onClick={e => e.stopPropagation()}>
            <input autoFocus value={renameValue} onFocus={(e) => e.target.select()} onChange={e => setRenameValue(e.target.value)} onBlur={(e) => { if (e.relatedTarget && e.relatedTarget.closest('.inline-edit-box')) return; submitRename(); }} onKeyDown={e => { if (e.key === 'Enter') submitRename(); if (e.key === 'Escape') setIsRenaming(false); }} style={{ padding: '0 4px', fontSize: 13 }} />
