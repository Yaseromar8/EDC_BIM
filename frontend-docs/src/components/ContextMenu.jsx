...
export default function ContextMenu({
  ...
  return (
    ...
    >
      {/* AÑADIR SUBCARPETA con «Editar» en la carpeta, que es lo que exige el
          servidor (13-sep-2026); antes solo a quien administraba la obra. */}
      {item.type === 'folder' && visible('subcarpeta') && (
        <button {...props('subcarpeta')} onClick={() => { if (!cap('subcarpeta').disponible) return; onClose(); onCreateChild(item.id || item.fullName); }}>
      ...
      {/* SUBIR NUEVA VERSIÓN: el mismo documento, otra versión. Con «Editar».
          Es lo que hace quien corrige lo que le rechazaron. */}
      {item.type !== 'folder' && onNuevaVersion && visible('nueva_version') && (
        <button {...props('nueva_version')} onClick={() => { if (!cap('nueva_version').disponible) return; onClose(); onNuevaVersion(item); }}>
          <div className="menu-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="17 8 12 3 7 8"></polyline><line x1="12" y1="3" x2="12" y2="15"></line></svg></div>
          Subir nueva versión
        </button>
      )}
      {visible('renombrar') && (
