...
def _obras_conocidas(*ids):
    """El middleware ya no acepta el project_id crudo: lo valida contra las
    obras reales. Se inyecta el mapa del resolutor para no tocar la BD."""
    import time
    import db
...
def test_authz_no_acepta_un_project_id_inventado(monkeypatch):
    """Un project_id que no corresponde a ninguna obra no vale como coartada.

    Antes se devolvia CRUDO lo que mandara el cliente, asi que la comprobacion
    validaba la obra que el atacante declaraba, no una obra real.
    """
    am, app = _make_app(monkeypatch, enforce=True)
    _obras_conocidas('1')
    am.validate_session = lambda t: {'id': 999, 'role': 'user'}
    llamadas = []
    am._user_in_project = lambda uid, pid: llamadas.append(pid) or False
    app.test_client().get('/api/tracking/x?project_id=OBRA_INVENTADA',
                          headers={'Authorization': 'Bearer N'})
    assert llamadas == [], 'se comprobo la membresia contra una obra inexistente'
