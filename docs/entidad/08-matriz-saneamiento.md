...
| **N2** | Alembic **nunca** se ejecuta en producción | `startCommand` real | **ABIERTO** | el esquema lo construye únicamente el DDL en caliente. **Sigue abierto**: arreglar el bootstrap (N57) hace que la reconstrucción funcione, pero producción no lo ejecuta — el `startCommand` real no lo llama, así que el esquema allí se sigue construyendo solo, en caliente |
