...
| **N68** | Un enlace de lectura caducado devuelve **403 sin replegarse a la sesión**: `_acceso_al_recurso` mira el `?t=` y, si no vale, corta —no comprueba si además hay sesión válida. El cliente cachea ese token 20 h y solo lo renueva al vencer. Una pestaña abierta en obra ve 403 en fotos y PDF hasta 20 horas **con sesión buena** | **ABIERTO** | `routes/documents.py:275-283`. Comparar con `pins.py:333-336` y `server.py:848-851`, que solo miran el token si NO hay sesión y por eso repliegan bien. Encontrado al verificar el efecto combinado de las dos variables de secreto |
