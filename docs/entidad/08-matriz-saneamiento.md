...
| **N64** | El **segundo factor estaba cerrado por la puerta que debía dejarlo pasar**: `POST /api/auth/2fa/verify` devolvía 401 `NO_TOKEN` en producción, antes del manejador. Se presenta entre la contraseña y la sesión, cuando por definición aún no hay sesión. El test que lo vigilaba comprobaba **solo la política declarada**, nunca la lógica que manda hoy: verde permanente sobre una ruta que respondía 401 | **CERRADO** `1ea10f2` | medido contra producción antes (401 `NO_TOKEN`) y después. 0 de 5 usuarios lo tenían activado, así que la trampa estaba armada y sin disparar: el primero que lo encendiera se quedaba fuera sin vuelta atrás salvo editar la base a mano. Mismo error ya pagado con `forgot-password`, escrito dos líneas más arriba en la misma lista. El candado ahora exige **los dos caminos**, no la intención |
