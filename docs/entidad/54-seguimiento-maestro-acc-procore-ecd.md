...
| 12 | **Identity & Access UX** | ACC account/project admin UI | Procore Directory UX | La escalera PERSONA→ENTIDAD→PROYECTOS→EMPRESA/FUNCIÓN→ADMINISTRACIÓN→PERMISOS, **sin un “rol” gigante** | **PARCIAL** — diseño CERRADO | LoginScreen (559 líneas, con invitación/reset/2FA) · Participantes · RolDeMiembro | **ABIERTO** — siguiente frente de Product Experience | **Implementación POST-WINDOW según doc 56** (G5→G7→G1–G3→pantallas; G4 aparte) |
