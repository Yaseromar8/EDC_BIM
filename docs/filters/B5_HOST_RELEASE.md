...
| 6. Guardar una V2 nueva con filtros/Sources/dos colores; cambiar escena, restaurar; probar V2-zero, luego edición manual y cambio de frente/reload; con color activo, abrir una lámina 2D y volver a 3D | Restore completo o degradación explícita justificada, jamás falso completo; un único frame final coherente. Tras zero se conservan predicados. Ni miembros/colores del frente anterior ni referencias al modelo retirado. Al volver de 2D no queda tinte de Filters en una instancia sustituida: es el único camino del visor que saca un modelo de `models()` sin descargarlo, y sólo HOST muestra qué dibuja LMV entonces | ID de vista de ensayo, parte de restore, video/frame y comparación de revisión |
